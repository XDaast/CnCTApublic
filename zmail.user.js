...
// @description Complete and comprehensive CCTA mail client. Zmail is fast and easy to use mail client that allows you to send and receive messages.
    ...
    function createClass() 
    {
		...
		qx.Class.define('zmail.structure',
		{
			...
			//folders & contacts		
					...
					var officersTxt = document.createTextNode('Officers');
					var alliesTxt = document.createTextNode('Allies');
					var groupsTxt = document.createTextNode('Groups');
			...
			//selected contacts container
					...
					text(sc_saveGroup, 'Save group');
					text(sc_saveNewGroup, 'New group');
					text(sc_saveExistedGroup, 'Existed group');
			...
			//tools bar		
					var toolsConstructor = function()
					{
						...
						text(notDocument, 'Move to Inbox');
						text(fwdMsg, 'Forward');
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					var createComposeContacts = function()
					{
						try
						{
							...
							text(c_contactsLi, 'Contacts');
							var c_contactsList = ['Alliance', 'Commanders', 'Officers', 'Allies', 'Friends', 'Groups'];
							c_contactsList.map(function(name)
							{
								...
								{
									var isOpen = (selectSpan.style.display == 'block');
									if ((itemsCont.children.length > 0) && !isOpen) selectSpan.style.display = 'block';
									if (isOpen) selectSpan.style.display = 'none';
								};
								...
								switch(name)
								{
									case 'Friends': root.contacts.friends.map(function(x){contacts.push(root.getPlayerNameById(x))}); break;
									case 'Allies':
										var arr = data.allies;
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					this.dom.compose.rightBar.msgSb = c_msgSb;
					...
					c_msgBody.onload = function()
					{
						...
						addCSSRule(sheet, "html", "color:#848585; font-family:vrinda; font-size:14px;", 0);
						addCSSRule(sheet, "body", "width:100%; margin:0; padding:0 0 10px 0; word-wrap:break-word; line-height: 18px;", 1);
						addCSSRule(sheet, "ul", "margin:0; color:#848585;", 2);
						addCSSRule(sheet, "ol", "margin:0; color:#848585;", 3);
						addCSSRule(sheet, "li", "font-family:vrinda; color:#848585;", 4);
						addCSSRule(sheet, "a", "font-family: vrinda", 5);
						addCSSRule(sheet, "div", "font-family:vrinda; color:#848585;", 6);
						addCSSRule(sheet, "blockquote", "font-family:vrinda; color:#848585;", 7);
						addCSSRule(sheet, "table", "border-color: #848585; color: #848585; font-family: vrinda; font-size: 14px;", 8);
						addCSSRule(sheet, "th", "background-color: #848585; color: #333; padding: 3px 15px; font-weight: normal; color: #292929; font-family: vrinda; font-size: 14px;", 9);
						addCSSRule(sheet, "td", "padding: 4px 15px; color: #848585; font-family: vrinda; font-size: 14px; word-wrap: break-word;", 10);
						...
						c_msgBody.contentWindow.onfocus = function()
						{
							...
							this.document.body.style.color = '#848585';
						};
						c_msgBody.contentWindow.onblur = function()
						{
							if(ibody.innerHTML == '' || ibody.innerHTML == '<br>')
							{
								ibody.innerHTML = 'Add Text';
								this.document.body.style.color = '#666';
							}
						};
			...
			members:
			{
				...
				update: function(arr1, arr2, arr3, arr4)
				{
					...
					this.dom.leftBar.contacts.groups.nodeValue = 'Groups ' + count('groups');
				...
				resetMsgCont: function(type)
				{
					...
					if ((type == 1) && !this.selectedGroup)
					{
						...
						header.innerHTML = (count > 0) ? count + ' messages selected.' : 'No messages selected.';
				...
				updateContacts: function(alliance, commanders, officers)
				{
					...
					allianceTxt.nodeValue = (alliance && (alliance.length > 0)) ? 'Alliance ' + alliance.length : 'Alliance';
					commandersTxt.nodeValue = (commanders && (commanders.length > 0)) ? 'Commanders ' + commanders.length : 'Commanders';
					officersTxt.nodeValue = (officers && (officers.length > 0)) ? 'Officers ' + officers.length : 'Officers';
				...
				printMsg: function(msg)
				{
					...
					var createSection = function(f,d,m,i,c)
					{
						...
						s_remove.onclick = function()
						{
							...
							var sb = root.dom.compose.rightBar.msgSb;
					...
					if((msg.fi !== -1) && (/<cnc>/.test(b)))
					{
						...
						for(var n = 0; n < sections.length; n++)
						{
							...
							{
								var to = create('p', cssStyles.message.from);
								var expandBox = create('span', cssStyles.message.expand.wrapper);
								var expand = create('div', cssStyles.message.expand.cont);
								...
								msgCont.zm_append([subject, from, to, msgBody]);
								if (to.scrollWidth > to.offsetWidth) to.appendChild(expandBox);
							}
				...
				addSelectedContacts: function()
				{
					...
					var text = sc.length == 0 ? 'No contacts selected.' : sc.length + ' contacts selected';
				...
				populateContacts: function()
				{
					try
					{
						...
						if ((contacts)&&(group != 'allies')&&(group != 'groups'))
						{
							...
							if (contacts.length * 60 > headersSbc.offsetHeight)
							{
								var sb = create('div', cssStyles.middleBar.scrollBar.bar);
								headersSbc.appendChild(sb);
								this.enableScroll(sb, headersSbc, headersCont);
						...
						else if ((contacts)&&((group == 'allies')||(group == 'groups')))
						{
							...
							selectAll.style.display = 'none';
							if ((root.size(contacts) * 60) > headersSbc.offsetHeight)
							{
								var sb = create('div', cssStyles.middleBar.scrollBar.bar);
								headersSbc.appendChild(sb);
								this.enableScroll(sb, headersSbc, headersCont);
						...
						else selectAll.style.display = 'none';
					}
					catch(e)
					{
						console.log('zmail: ' + e.toString());
